...
export default React.createClass({
        ...
        return (
            <span className="pull-right refresh-button">
                <RefreshComponent
                    ...
                    timestamp={stores.InstanceHistoryStore.lastUpdated}
                    delay={1000 * 60}
                />
            </span>
        );
    ...
    renderTitle: function() {
        ...
        if (instanceHistories) {
            historyCount =
                " (" + instanceHistories.meta.count + " instances launched)";
            title += historyCount;
        }
    ...
    renderBody: function() {
        ...
        if (!instanceHistories || !instances || !providers)
            return <div className="loading" />;

        instanceHistoryItems = instanceHistories.map(
            function(item) {
                ...
                var startDate = item.get("instance").start_date,
                    ...
                    formattedEndDate = moment(endDate).format(
                        "MMM DD, YYYY hh:mm a"
                    ),
                    now = moment(),
                    timeSpan = now.diff(startDate, "days"),
                    instanceHistoryHash = CryptoJS.MD5(
                        (instanceId || item.cid).toString()
                    ).toString(),
                    iconSize = 63,
                ...
                if (image) {
                    imageLink = (
                        <Link to={`images/${image.id}`}>{imageName}</Link>
                    );
                } else {
                    imageLink = <strong>{imageName}</strong>;
                }
        ...
        return (
            <div onClick={this.loadMoreHistory}>
                <h2 className="t-headline">
                    {this.renderTitle()} {this.renderRefreshButton()}
                </h2>
                {this.renderBody()}
            </div>
        );
