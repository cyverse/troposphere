...
export default React.createClass({
    ...
    componentDidMount: function() {
        ...
        var instanceOptions = stores.InstanceStore.getAll().map(function(instance) {
            return (
            <option key={instance.id} value={instance.get("size").cpu}>
                {instance.get("name")}(
                {instance.get("size").cpu} CPU)
            </option>
            );
        });
        ...
        this.setState({
            options: options,
            selectedCPU: cpuToSet
        });
    },

    handleCPUChange: function(e) {
        this.setState({
            selectedCPU: Number(e.target.value)
        });
    },
        ...
        <div>
            <div className="form-group">
                <a role="button"
                    ...
                    href="#au-calculator"
                    aria-expanded="true">AU Calculator</a>
            </div>
            <div id="au-calculator" className="collapse">
                ...
                <table className="table">
                    ...
                    <tbody>
                        <tr className={remainingAU >= this.state.selectedCPU * 24 * 1 ? "success" : "warning"}>
                            <td>
                                1 day
                            </td>
                            <td>
                                {(this.state.selectedCPU * 24 * 1)}
                            </td>
                        </tr>
                        <tr className={remainingAU >= this.state.selectedCPU * 24 * 3 ? "success" : "warning"}>
                            <td>
                                3 days
                            </td>
                            <td>
                                {(this.state.selectedCPU * 24 * 3)}
                            </td>
                        </tr>
                        <tr className={remainingAU >= this.state.selectedCPU * 24 * 7 ? "success" : "warning"}>
                            <td>
                                1 week
                            </td>
                            <td>
                                {(this.state.selectedCPU * 24 * 7)}
                            </td>
                        </tr>
                        <tr className={remainingAU >= this.state.selectedCPU * 24 * 14 ? "success" : "warning"}>
                            <td>
                                2 weeks
                            </td>
                            <td>
                                {(this.state.selectedCPU * 24 * 14)}
                            </td>
                        </tr>
                    </tbody>
                </table>
