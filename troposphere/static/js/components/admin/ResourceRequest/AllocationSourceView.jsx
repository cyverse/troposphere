...
export default React.createClass({
    ...
    renderAllocation(allocationSource) {
        ...
        if (globals.EXTERNAL_ALLOCATION) {
            return <AllocationView key={name} {...{allocationSource}} />;
        }
        let props = {allocationSource, onSave};
        return <AllocationEditableSaveableView key={name} {...props} />;
    },

    render() {
        let {allocationSource} = this.props;

        return <div>{this.renderAllocation(allocationSource)}</div>;
    }
});

class AllocationView extends React.Component {
    getUsage(allocation) {
        let {compute_used, compute_allowed} = allocation.pick([
            "compute_used",
            "compute_allowed"
        ]);
        let percent = 0;
        ...
        if (compute_allowed !== 0) {
            ...
            percent =
                Math.round(compute_used / compute_allowed * 100 * 100) / 100;
...
class AllocationEditableSaveableView extends AllocationView {
    ...
    onSave() {
        ...
        // setState on unmount
        let onSuccess = cancellable(allocation => {
        ...
        // our state
        this.props.onSave(allocationSource.clone()).then(onSuccess, onErr);
    }
    ...
    renderSaveText() {
        ...
        return (
            <span>
                {" "}
                <a style={{float: "right"}} onClick={this.onSave.bind(this)}>
                    {buttonText}
                </a>
            </span>
        );
    ...
    render() {
        ...
        let {name} = this.props.allocationSource.pick("name");
        let editableTotal = isEditing ? this.renderEditableField(total) : total;
        ...
        let hasChanged =
            ...
            allocationSource
                ...
                .some(
                    attr =>
                        allocationSource.get(attr) !== cloudAllocation.get(attr)
                );
