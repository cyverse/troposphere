...
const ImageMaster = React.createClass({
    ...
    renderRefreshButton: function() {
        var controlsClass =
            "glyphicon pull-right glyphicon-refresh" +
            (this.state.refreshing ? " refreshing" : "");

        return <span className={controlsClass} onClick={this.onRefresh} />;
    },
    ...
    renderRequestView() {
        ...
        if (!("id" in params)) {
            return <p>Please select a request.</p>;
        }
        ...
        if (!request) {
            return (
                <p>
                    This request is no longer active. Please select a request.
                </p>
            );
        }
    ...
    render: function() {
        ...
        var imageRequestRows = requests.map(
            function(request) {
                ...
                var errorStatus,
                    ...
                    ownerUsername = newMachineOwner
                        ? newMachineOwner.username
                        : "(unknown)";

                if (
                    request.get("old_status").indexOf("ERROR") > -1 ||
        ...
        return (
            <div className="image-master">
                <h2 className="t-headline">
                    Imaging Requests {this.renderRefreshButton()}
                </h2>
                <ul className="requests-list pull-left">{imageRequestRows}</ul>
