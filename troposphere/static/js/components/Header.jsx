...
let LogoutLink = React.createClass({
    ...
    onLogout: function(e) {
        e.preventDefault();
        var api_logout_url = globals.API_V2_ROOT.replace(
            "/api/v2",
            "/api-auth/logout/"
        );
        $.ajax(api_logout_url, {
        ...
        return (
            <li className="dropdown">
                <a className="dropdown-toggle" href="#" data-toggle="dropdown">
                    {expiredBadge}
                    {username} <b className="caret" />
                </a>
                <ul className="dropdown-menu">
                    {expiredMenuItem}
                    <li>
                        <Link to="settings" onClick={trackSettings}>
                            {" "}
                            Settings
                        </Link>
                    </li>
                    <li>
                        <Link
                            to="my-requests/resources"
                            onClick={trackRequests}>
                            {" "}
                            My requests
                        </Link>
                    </li>
                    <li>
                        <a
                            id="version_link"
                            href="#"
                            onClick={this.onShowVersion}>
                            Version
                        </a>
                    </li>
                    {statusPageEl}
                    <li>
                        <a id="logout_link" href="#" onClick={this.onLogout}>
                            Sign out
                        </a>
                    </li>
                </ul>
    ...
    renderNavLinks() {
        ...
        return navLinks.map(link => {
            ...
            return (
                <li
                    key={link.name}
                    data-toggle={toggleMenu.toggle}
                    data-target={toggleMenu.target}>
                    <Link to={link.linksTo} activeClassName="active">
                        <i className={"glyphicon glyphicon-" + link.icon} />
                        {link.name}
                    </Link>
                </li>
            );
        ...
        return (
            <div
                ...
                <MaintenanceMessageBanner
                    maintenanceMessages={this.props.maintenanceMessages}
                />
