...
export default React.createClass({
    ...
    render: function() {
        ...
        if (stores.ProfileStore.get().get("is_staff")) {
            machineStateColumn = <th>Machine State</th>;
        }
            ...
            return (
                <div className="container">
                    <p style={{marginBottom: "16px"}}>
                        ...
                        <a href={imagingDocsLink.get("href")} target="_blank">
        ...
        var displayRequests = requests.map(
            function(request) {
                ...
                if (request.get("end_date")) {
                    endDateText = moment(request.get("end_date")).format(
                        "MMM D, YYYY h:mm:ss a"
                    );
                }

                var newMachineId = request.get("new_machine")
                    ? request.get("new_machine").uuid
                    : "N/A";
                var newMachineProvider = request.get("new_machine_provider")
                    ? request.get("new_machine_provider").name
                    : "Unknown";

                const {name, uuid} = request.get("instance");
                const requestDate = moment(request.get("start_date")).format(
                    "MMM D, YYYY h:mm:ss a"
                );
