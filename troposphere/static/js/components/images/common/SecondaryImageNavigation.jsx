...
export default React.createClass({
    ...
    renderRoute: function(name, linksTo, icon, requiresLogin) {
        ...
        return (
            <li key={name}>
                <Link to={`/images/${linksTo}`} activeClassName="active">
                    <Glyphicon name={icon} />
                    <span>{name}</span>
                </Link>
            </li>
    ...
    render: function() {
        ...
        if (!userLoggedIn) {
            ...
            if (!userImages || !favoritedImages) {
                ...
                return (
                    <div style={{height: "63px"}} className="secondary-nav" />
                );
            }
            ...
            <div>
                <div className="secondary-nav">
                    <div className="container">
                        <ul className="secondary-nav-links">{routes}</ul>
                    </div>
                </div>
            </div>
        );
