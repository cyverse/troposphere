    ...
    render: function() {
        ...
        var instance = this.props.instance,
            ...
            image = instance_image
                ? stores.ImageStore.get(instance_image.id)
                : null; //Bootable volumes have no image ID..
        ...
        label = version_name
            ? image_name + " " + version_separator + version_name
            : image_name;

        return (
            <ResourceDetail label="Based on">
                <Link to={`images/${image.id}`}>{label}</Link>
            </ResourceDetail>
        );
    }
});
