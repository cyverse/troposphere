...
const Project = React.createClass({
    ...
    getLeaderNames: function(project) {
        ...
        let username_list = leader_list.map(function(g) {
            return g.username;
        });
    ...
    renderIdentityList: function() {
        ...
        let {IdentityStore} = this.props.subscriptions,
            ...
            ownedIdentityList = IdentityStore.getIdentitiesForGroup(
                group,
                current_user
            );
        ...
        let ownedIdentityNames = ownedIdentityList.map(function(i) {
            return i.toString();
        });
        return (
            <p className="t-caption" style={{display: "block"}}>
    ...
    render: function() {
        ...
        if (project && project.id && !project.isNew()) {
            ...
            projectCreationDate = moment(project.get("start_date")).format(
                "MMM D, YYYY hh:mm a"
            );
...
export default subscribe(Project, ["IdentityStore"]);
