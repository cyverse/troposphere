  ...
  return React.createClass({
    ...
    render: function() {
      ...
      var displayRequests = requests.map(function(request){
        ...
        // Handler to allow close buton to properly call React class closeRequest with the proper argument
        var close = function(){
        ...
        if(text === 'approved'){
          ...
          closeButton = <button type="button" className="btn btn-default" onClick={close}>Close</button>;
        }

        return (
          <tr key={request.id} className={trClass}>
            <td>{request.get('request')}</td>
            <td>{request.get('description')}</td>
            <td>{request.get('status').name} {closeButton}</td>
          </tr>
        );
