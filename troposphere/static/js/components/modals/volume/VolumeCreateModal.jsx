...
export default React.createClass({
    ...
    isSubmittable: function() {
        ...
        var selectedIdentity = stores.IdentityStore.get(this.state.identityId),
            isProviderInMaintenance = stores.MaintenanceMessageStore.isProviderInMaintenance(
                selectedIdentity.get("provider").id
            );
        ...
        var hasProvider = !!this.state.identityId,
            ...
            hasEnoughQuotaForStorage = this.hasEnoughQuotaForStorage(
                selectedIdentity,
                this.state.volumeSize,
                volumes
            ),
            hasEnoughQuotaForStorageCount = this.hasEnoughQuotaForStorageCount(
                selectedIdentity,
                volumes
            );
        ...
        return (
            <div className="quota-consumption-bars">
                <div className="progress">
                    ...
                    <div
                        className={"progress-bar " + barTypeClass}
                        style={currentlyUsedStyle}
                    />
                    <div
                        className={"progress-bar " + barTypeClass}
                        style={projectedUsedStyle}
                    />
                </div>
                <div>{message}</div>
            </div>
    ...
    renderStorageConsumption: function(identity, size, volumes) {
        ...
        var message =
            "You will use " +
            Math.round(currentlyUsed + projected) +
            " of " +
            maximumAllowed +
            " allotted GBs of Storage.";
        var overQuotaMessage = (
            <div>
                <strong>Storage quota exceeded.</strong>
                <span>
                    {" Choose a smaller size or delete an existing volume."}
                </span>
            </div>
        );
    ...
    renderStorageCountConsumption: function(identity, size, volumes) {
        ...
        var overQuotaMessage = (
            <div>
                ...
                <span>
                    {
                        " You must delete an existing volume before creating a new one."
                    }
                </span>
            </div>
        );
    ...
    renderBody: function() {
        ...
        if (!identities || !providers || !volumes)
                ...
                <div className="modal-section">
                    <h4 className="t-body-2">
                        ...
                        <a
                            ...
                            href="#"
                            onClick={this.handleResourceRequest}>
                            {"Need more resources?"}
                        </a>
                    </h4>
                    {this.renderStorageConsumption(identity, size, volumes)}
                    {this.renderStorageCountConsumption(
                        identity,
                        size,
                        volumes
                    )}
