...
const NullProjectMigrateResourceModal = React.createClass({
    ...
    isSubmittable: function() {
        ...
        var hasValidProjectMapping = flattened.every(function(
            project_resource
        ) {
    ...
    renderSharingText: function() {
        ...
        let sharingText =
            "NEW: You can now share your cloud resources with other users based on your assigned groups " +
        ...
        return (
            <p className="alert alert-info" style={{fontWeight: 500}}>
                {sharingText}
            </p>
        );
    },
    ...
    renderExplanationText: function() {
        ...
        if (projects.length > 0) {
            explanationText =
                "In order to interact with your resources (such as suspending instances or attaching " +
    ...
    resourceOptionName: function(project) {
        ...
        let isGroupLeader = groupLeaders.find(
    ...
    renderGroupResourceSelection: function() {
        ...
        groups.forEach(group => {
            let identities_for_group = IdentityStore.getIdentitiesForGroup(
                group
            );
        ...
        let that = this;
        let groupBasedSelection = _.map(Object.keys(resourcesByGroup), function(
            group_id
        ) {
            ...
            let resourceSelectionOptions = groupResources.map(function(
                resource
            ) {
                return that.renderResourceProjectSelection(
                    resource,
                    identities_for_groups
                );
            });
            return (
                <div key={"group-select-" + group_id}>
                    ...
                    <ul>{resourceSelectionOptions}</ul>
                </div>
            );
        });
    ...
    renderResourceSelection: function() {
        ...
        let that = this;
        let resourceSelectionList = this.props.resources.map(function(
            resource
        ) {
            return that.renderResourceProjectSelection(
                resource,
        ...
        return (
            <div className="form-group" style={{overflowY: "auto"}}>
                <ul style={{paddingLeft: 0}}>{resourceSelectionList}</ul>
            </div>
        );
    },
...
export default subscribe(NullProjectMigrateResourceModal, [
    "GroupStore",
    "ProjectStore",
    "IdentityStore"
]);
