...
export default {
    _migrateResourceIntoProject: function(resource, project) {
        ...
        if (resource instanceof Instance) {
            Utils.dispatch(
                NullProjectInstanceConstants.REMOVE_INSTANCE_FROM_NULL_PROJECT,
                {
                    instance: resource
                }
            );
            ...
            Utils.dispatch(
                NullProjectVolumeConstants.REMOVE_VOLUME_FROM_NULL_PROJECT,
                {
                    volume: resource
                }
            );
    ...
    _migrateResourceIntoRealProject: function(
        resource,
        oldProject,
        newProject
    ) {
    ...
    moveAttachedVolumesIntoCorrectProject: function() {
        ...
        volumes.each(
            function(volume) {
                var volumeProjectId = volume.get("project")
                        ? volume.get("project").id
                        : -1,
                ...
                if (instanceUUID) {
                    ...
                    if (volumeProjectId !== instanceProjectId) {
                        ...
                        this._migrateResourceIntoRealProject(
                            volume,
                            volumeProject,
                            project
                        );
        ...
        if (volumesInWrongProject.length > 0) {
            ...
            ModalHelpers.renderModal(
                NullProjectMoveAttachedVolumesModal,
                props,
                function() {}
            );
        }
    ...
    migrateResourcesIntoProject: function(nullProject) {
        ...
        if (resources.length > 0) {
            ...
            ModalHelpers.renderModal(
                NullProjectMigrateResourceModal,
                props,
                function(project_resource_list) {
