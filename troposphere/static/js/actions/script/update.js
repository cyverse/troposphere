...
export default {
    update: function(script, newAttributes) {
        if (!newAttributes.title) throw new Error("Missing title");
        if (!newAttributes.type) throw new Error("Missing type");
        if (!newAttributes.text) throw new Error("Missing text");
        if (!newAttributes.strategy) throw new Error("Missing strategy");
        ...
        script.save(newAttributes, {patch: true}).fail(response => {
            Utils.displayError({
